...
function generateComponentsList(components) {
    let list = [];
    components.map(nav => nav.list.map(item => list.push(item.path.substr(1))));
    return list;
}
...
function compilingComponentLog() {
    return through.obj((file, encode, callback) => {
        if (file.path) {
            ...
            file.path.replace(/lib\/((\S)+)/gi, (s, v) => {
                if (!v) return;
                if (res < 0) {
                    succNum++;
...
function gulpVueify(options) {
    return through.obj(function(file, encode, callback) {
        ...
        if (file.isStream()) {
            this.emit(
                'error',
                new gutil.PluginError('gulp-vueify', 'Streams are not supported')
            );
            return callback();
        }
        if (options) {
...
gulp.task('compile', () => {
    ...
    const streamCompiledStylus = gulp
        .src('./components/_style/*.styl')
        .pipe(
            gulpStylus({
                compress: true,
                import: mixins,
